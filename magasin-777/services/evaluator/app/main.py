...
from app.core.database import Base, SessionLocal, engine, prepare_schema
...
def validate_required_secrets() -> None:
    """Fail closed: production credentials must never use built-in defaults."""
    import os
    missing = [name for name in ("ADMIN_PASSWORD", "SECRET_KEY") if not os.getenv(name, "").strip()]
    if missing:
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[x for x in os.getenv("CORS_ALLOW_ORIGINS", "http://localhost").split(";") if x],
...
def _seed_admin(db):
    ...
    # Existing accounts/passwords/roles are NEVER reset on startup.
